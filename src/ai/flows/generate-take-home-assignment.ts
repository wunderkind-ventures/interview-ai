...
2.  **Assignment Structure (Strictly Adhere to this Format):**
    ...
    *   **### Key Aspects to Consider / Guiding Questions**:
        *   List 5-8 bullet points or explicit questions tailored to the specific 'Problem Scenario', 'interviewFocus', and 'faangLevel' generated above. These are not to be answered one-by-one necessarily, but to guide the candidate's thinking and ensure their response covers critical dimensions.
...
When crafting the assignment, particularly the "Goal / Objective" and "Problem Scenario", subtly weave in opportunities for the candidate to demonstrate understanding or application of Amazon's Leadership Principles, especially if relevant to the 'interviewFocus' and 'faangLevel'. For example, if the focus is on a new product, Customer Obsession is key. If it's about scaling a system, Insist on the Highest Standards or Dive Deep might be relevant. An L6 might be expected to 'Think Big'.
