...
When crafting the assignment, particularly the "Goal / Objective" and "Problem Scenario", subtly weave in opportunities for the candidate to demonstrate understanding or application of Amazon's Leadership Principles, especially if relevant to the 'interviewFocus' and 'faangLevel'. For example, if the focus is on a new product, Customer Obsession is key. If it's about scaling a system, Insist on the Highest Standards or Dive Deep might be relevant. An L6 might be expected to 'Think Big'. For 'machine learning' assignments, 'Learn and Be Curious' or 'Are Right, A Lot' (in terms of justifying model choices and experimental design) can be relevant.
